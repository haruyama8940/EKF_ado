...
vt = ut(1);                             % 速度
wt = ut(2);                             % 角速度
Gt = [1, 0, ((-vt*cos(theta))/wt)+((vt*cos(theta+wt*dt))/wt); ...
      0, 1, ((-vt*sin(theta))/wt)+((vt*sin(theta+wt*dt))/wt); ...
      0, 0, 1];
...
mu_tb = mu_t1 + [((-vt*sin(theta))/wt)+((vt*sin(theta+wt*dt))/wt), ((vt*cos(theta))/wt)-((vt*cos(theta+wt*dt))/wt), wt*dt]';
...
if (not(isempty(zt)))
  ...
  Ht = [-(m(1)-mu_tb(1))/q^(1/2), -(m(2)-mu_tb(2))/q^(1/2), 0;
